...
func checkPortForward() (string, error) {
	...
	for _, portRange := range portRanges {
		for port := portRange.start; port <= portRange.end; port++ {
			if host := testPortForKubeArchive(port); host != "" {
				return host, nil
			}
		}
	}
...
func testPortForKubeArchive(port int) string {
	httpsHost := fmt.Sprintf("https://localhost:%d", port)
	if TestKubeArchiveLivezEndpoint(httpsHost, true, nil) == nil {
		return httpsHost
	}
	return ""
}

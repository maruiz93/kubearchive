...
func NewMockKACLICommand(mockErr error, resourceInfo *ResourceInfo) *MockKACLICommand {
...
func (m *MockKACLICommand) ResolveResourceSpec(resourceSpec string) (*ResourceInfo, error) {
	return m.mockResourceInfo, nil
}

func (m *MockKACLICommand) GetFromAPI(api API, _ string) ([]byte, *APIError) {
	switch api {
	case Kubernetes:
		if m.k8sError != nil {
...
func (m *MockKACLICommand) AddRetrieverFlags(_ *pflag.FlagSet) {}
...
func (m *MockKACLICommand) CompleteRetriever() error {
	return m.completeError
}

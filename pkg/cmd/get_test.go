...
func TestGetComplete(t *testing.T) {
	testCases := []struct {
		...
		afterStr        string // empty string means no --after flag provided
		beforeStr       string // empty string means no --before flag provided
		...
		},
		{
			...
			expectedApiPath: "/api/v1/namespaces/default/pods",
			afterStr:        "2023-01-01T00:00:00Z",
			beforeStr:       "2023-12-31T23:59:59Z",
			limit:           100,
		},
		{
			...
			args:          []string{"pods"},
			expectError:   true,
			errorContains: "invalid --after format",
			afterStr:      "invalid-timestamp",
		},
		{
			name:          "invalid before timestamp format",
			args:          []string{"pods"},
			expectError:   true,
			errorContains: "invalid --before format",
			beforeStr:     "invalid-timestamp",
		},
		{
			...
			afterStr:      "2023-12-31T23:59:59Z",
			beforeStr:     "2023-01-01T00:00:00Z",
		},
		{
			...
			limit:         -5, // Negative value is invalid
		},
		{
	...
	for _, tc := range testCases {
		t.Run(tc.name, func(t *testing.T) {
			...
			if tc.afterStr != "" {
				options.afterStr = tc.afterStr
			}
			if tc.beforeStr != "" {
				options.beforeStr = tc.beforeStr
			}
			if tc.limit != 0 {
